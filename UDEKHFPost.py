...
class UDExtendedKalmanHinfFilterPosterior(UDExtendedKalmanFilter):
    ...
    def _scalar_update(self, axis_residual, axis_hjacobian, r):
        ...
        if a >= 0.0:
            ...
            c = f.dot(v_tilde)
            q = a/(c*c)
            WW = np.concatenate((u, dot(u, v_tilde).reshape((n,1))), axis = 1)
            DD = np.concatenate((d, np.array([q])))
            u, d = mwgs(WW, DD)
            #Now must recompute x-update
